...
export class UrgencyBannerManager {
  ...
  mountBanner(bannerConfig) {
    ...
    this.openBanners.set(bannerConfig.id, { el, key, mode: bannerConfig.dismissMode });
  ...
  startCountdown(el, countdownConfig) {
    ...
    if (Number.isNaN(endTime)) {
      reportError('urgency-banner.countdown', new Error('Invalid endsAt date'), { endsAt: countdownConfig.endsAt });
      return;
    }
